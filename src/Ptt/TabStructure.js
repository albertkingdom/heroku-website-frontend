...
function TabStructure({ mostpush, browseWithPics, allArticle }){
    useEffect(()=>{
        ...
        function removeActive(){
            document.querySelectorAll('div.tab').forEach(item =>item.classList.remove('active'))
            document.querySelectorAll('li.active').forEach(item =>item.classList.remove('active'))
        }
    ...
    return (
        ...
        <ul className="tabControlBtn">
            <li id="tab1btn" className="active">最多推文</li>
